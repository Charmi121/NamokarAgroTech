					...
					if (!empty($_GET['edit']) && (int)$_GET['edit'] == 1)
					{
						...
						if (count($rsproducts)>0) {
							foreach($rsproducts as $rowproduct){
								...
								$horsepower    =   $rowproduct['horsepower'];
								$machine_width    =   $rowproduct['machine_width'];
								$hitch_types    =   $rowproduct['hitch_types'];
								//$category_id         =   (int)$rowproduct['category_id'];
								$selectedcat         =   array(0);
								//$selectedcities      =   array(0);
								$product_name        =   trim($rowproduct['product_name']);
								$tech_description	 =   html_entity_decode($rowproduct['tech_description'],ENT_QUOTES,"utf-8");
								$advantage	   	   	 =   html_entity_decode($rowproduct['advantage'],ENT_QUOTES,"utf-8");
								$feature	   	   	 =   html_entity_decode($rowproduct['feature'],ENT_QUOTES,"utf-8");
								...
								$is_new              =   (int)$rowproduct['is_new'];
								$video_url           =   trim($rowproduct['video_url']);
								$pdf_file            =   trim($rowproduct['pdf_file']);
					...
					<form name="addform" id="addform" method="post" action="productsave.php"  enctype="multipart/form-data" target="_self">
						...
						<table class="table table-bordered table-striped table-condensed">
							<tbody>
											...
											<select id="category_id" name="category_id[]" class="category" style="width:100%;" placeholder="Select Category" >
												...
												<?php
													...
													if (count($rsparentcategories)>0) {
														...
														<option value="<?php echo $rowparentcategory['id'];?>" <?php if(in_array($rowparentcategory['id'], $selectedcat)) { echo "selected=\"selected\""; } ?>><?php echo $rowparentcategory['category_name'];?> </option> 
								...
								<?php /*
									<tr>
									<td><div class="text-right">Advantages</div></td>
									<td>
									<div class="col-md-12">
                                    <textarea cols="50" rows="2" name="advantage" id="advantage" placeholder="Please enter advantage" class="form-control inbox-editor inbox-wysihtml5"><?php echo $advantage; ?></textarea>
									</div>
									</td>
									</tr>
									<tr>
									<td><div class="text-right">Features</div></td>
									<td>
									<div class="col-md-12">
                                    <textarea cols="50" rows="2" name="feature" id="feature" placeholder="Please enter features" class="form-control inbox-editor inbox-wysihtml5"><?php echo $feature; ?></textarea>
									</div>
									</td>
									</tr>
									<tr>
									<td><div class="text-right">Technical Description</div></td>
									<td>
									<div class="col-md-12">
                                    <textarea cols="50" rows="2" name="tech_description" id="tech_description" placeholder="Please enter technical description" class="form-control inbox-editor inbox-wysihtml5"><?php echo $tech_description; ?></textarea>
									</div>
									</td>
									</tr>
								*/ ?>
								<tr>
									<td class="col-md-2"><div class="text-right">PDF File</div></td>
									<td class="col-md-10">
										<div class="col-md-12">
											<input type="file" name="pdf_file" id="pdf_file" />
											<?php if(!empty($pdf_file)) { ?>
												<a href="<?php echo DISPLAY_PATH."/product_brochure/".$pdf_file; ?>" target="_blank" class="btn btn-sm btn-info"> Download</a>
											<?php } ?>
										</div>
									</td>
								</tr>
								<tr>
									<td class="col-md-2"><div class="text-right">Is New?</div></td>
									<td class="col-md-10">
										<div class="col-md-12">
											<input type="checkbox" name="is_new" id="is_new" value="1" <?php echo (!empty($is_new)) ? 'checked' : ''; ?> />
										</div>
									</td>
								</tr>
								<?php /* 
								...
								*/?>
								...
								<?php if($edit == 0){ ?>
									...
									<tr>
										<td><div class="text-right">Upload Product Image<br/></div></td>
										<td>
											<div class="col-md-6">
									...
									<?php for($icount = 2; $icount <= 11; $icount++) { ?>
										<tr>
											<td><div class="text-right"><?php if($icount == 2) { ?>Upload Featured Image <?php } else if($icount == 3){ ?> Upload Banner Image<?php } else { ?>Upload Image <?php echo $icount - 3; ?><?php } ?></div></td>
											<td>
												<div class="col-md-5">
													<input type="file" name="big_image_<?php echo $icount; ?>" id="big_image_<?php echo $icount; ?>" class="control" /><span class="text-danger">Best Size (width x height): 2000px X 2000px</span>
												</div>
												<div class="col-md-5">
													<input type="text" name="alt_text_<?php echo $icount; ?>" id="alt_text_<?php echo $icount; ?>" placeholder="Alt" class="form-control" />
												</div>
											</td>
										</tr>
									<?php } ?>
									...
									<?php
										...
										if (count($rsproduct_images)>0) {
											...
											<?php if(!empty($rowproduct_image['mini_image']) && (int)$DB->checkFileExists(UPLOAD_PATH."/products/".$rowproduct_image['thumb_image']."") == 1){ ?>
												<tr>
													<td><div class="text-right">Previously <?php if($rowproduct_image['is_featured'] == 1) { ?> Uploaded Featured Image <?php } else if($rowproduct_image['is_banner'] == 1){ ?> Uploaded Banner Image<?php } else if($i == 1){ ?> Uploaded Product Image <?php } else { ?> Uploaded Image <?php echo $i - 3; ?><?php } ?></div></td>
											...
											<tr>
												<!--<td><div class="text-right"><?php if($rowproduct_image['is_featured'] == 1) { ?> Uploaded Featured Image <?php } else if($rowproduct_image['is_banner'] == 1){ ?> Upload Image <?php echo $i; ?><br/> <?php if($i == 1){ ?>( Default Image) <?php } ?><?php } ?></div></td>-->
												<td></td>
												<td>
													<div class="col-md-5">
													...
													<?php if($i > 1){ ?>
													<div class="col-md-5">
														<input type="text" name="alt_text_<?php echo $i; ?>" id="alt_text_<?php echo $i; ?>" value="<?php echo $rowproduct_image['alt_text'];?>" placeholder="Alt" class="form-control" />
													</div>
													<?php } ?>
									...
									<?php } ?>
									<?php for($icount = $i; $icount <= 11; $icount++) { ?>
										<tr>
											<td><div class="text-right"><?php if($icount == 2) { ?>Upload Featured Image <?php } else if($icount == 3){ ?> Upload Banner Image<?php } else { ?>Upload Image <?php echo $icount - 3; ?><?php } ?></div></td>
											<td>
												<div class="col-md-5">
								...
								<tr>
									<td class="col-md-2"><div class="text-right">Youtube video URL</div></td>
									<td class="col-md-10">
										<div class="col-md-9">
											<input type="text" name="video_url" id="video_url" value="<?php echo $video_url; ?>" class="form-control"  size="50"  title="Please enter video url" />
										</div>
									</td>
								</tr>
								<?php
								...
								<?php //} ?>
                                                                                                                                ...
                                                                                                                                <tr>
									<td class="col-md-2"><div class="text-right">Horsepower<span class="text-danger"></span></div></td>
									<td class="col-md-10">
										<div class="col-md-9">
											<input type="text" name="horsepower" id="horsepower" value="<?php echo $horsepower;?>" maxlength="250" class="form-control  control" title="Please enter horsepower"/>
										</div>
									</td>
								</tr>
									<td class="col-md-2"><div class="text-right">Machine Width<span class="text-danger"></span></div></td>
									<td class="col-md-10">
										<div class="col-md-9">
											<input type="text" name="machine_width" id="machine_width" value='<?php echo $machine_width;?>' maxlength="250" class="form-control control" title="Please enter machine width"/>
										</div>
									</td>
								</tr>
									<td class="col-md-2"><div class="text-right">Hitch Types<span class="text-danger"></span></div></td>
									<td class="col-md-10">
										<div class="col-md-9">
											<input type="text" name="hitch_types" id="hitch_types" value="<?php echo $hitch_types;?>" maxlength="250" class="form-control  control" title="Please enter hitch types"/>
										</div>
									</td>
								</tr>
