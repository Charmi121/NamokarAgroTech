<?php
    ...
    if(!empty($_FILES['big_image']) && floatval($_FILES['big_image']['size']) > 0)
    {
        ...
        //Thumb Image
        $magicianObj -> resizeImage(398, 578, array('crop', 'm'), true);
    ...
    if (isset($edit)){
        if ((int)$edit == 0){
            $sqlquery = $fpdo->insertInto('tblawards', array(
                                            'title' => $photo_title,                                            
            ...
            $sqlquery = $fpdo->update('tblawards')->set(array(
                                        'title' => $photo_title,                                        
                                        'status' => $status,
                                        'sort_order' => $sort_order                                       
                                   ));
            ...
            $sqlquery->where('tblawards.id = ?', $id);
            $sqlquery->execute();
    ...
    if ((int)$action == 1) {
        ...
        header("Location: displayaward.php");
        exit;
    } elseif ((int)$action == 2) {
