    ...
    if(!empty($_GET['delete'])) {
        ...
        if(!empty($filename)){
            ...
            if(trim($filetype) == "big_image" || trim($filetype) == "thumb_image") {
                $thumb_image  =  '';
                $big_image    =  ''; 
                $sqlquery     =  "UPDATE tblsliders SET thumb_image = '".$thumb_image."' AND big_image = '".$big_image."'";
        ...
        if ((int)$action == 1) {
            $_SESSION['sliderstatus'] = "filedeleted";
            header("Location: slider.php?id=".$id."&edit=1");
            exit;
        } else {
