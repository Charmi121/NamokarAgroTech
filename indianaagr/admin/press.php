...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            ...
            <?php if(!empty($_SESSION['pressstatus']) &&  $_SESSION['pressstatus']=="invalid" ) { ?>
                <div class="alert alert-danger" role="alert">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <h4>Invalid!</h4>
                    <?php echo $errorlist['press'][$_SESSION['pressstatus']]; ?>
                </div>
                <div class="clearfix"></div>
                <?php unset($_SESSION['pressstatus']);
                } elseif(!empty($_SESSION['pressstatus']) &&  $_SESSION['pressstatus']!="invalid") {
                ?>
            ...
            <div class="middlesection margin-top20">
                ...
                <?php
                    if (!empty($_GET['edit']) && (int)$_GET['edit'] == 1) {
                        ...
                        $id     =   (int)$_GET['id'];
                         ...
                         $sqlquery = $fpdo->from('tblpresses')
                                          ->where('tblpresses.id =:id ',array( ":id"=>$id) );
                        $rsdata   = $sqlquery->fetchAll();
                        if(count($rsdata) > 0) {
                            foreach($rsdata as $rowdata ) {
                                $press_name     =   trim($rowdata['press_name']);
                ...
                ?>
                <form name="addform" id="addform" method="post" action="presssave.php" enctype="multipart/form-data" target="_self">
                    ...
                    <table class="table table-bordered table-striped table-condensed">
                        <tbody>
                            <tr>
                                <td class="col-md-3"><div class="text-right">Press Name<span class="text-danger">*</span></div></td>
                                <td class="col-md-9">
                                    <div class="col-md-9">
                                        <input type="text" name="press_name" id="press_name" value="<?php echo $press_name; ?>" maxlength="150" class="required form-control" placeholder="Enter Press Name" title="Please enter press name"/>
                                    </div>
                                </td>
                            </tr>
                            
                            <?php if(!empty($big_image) && (int)$DB->checkfileexist("../uploads/presses/".$big_image."") == 707 ){ ?>
                                <tr>
                                    <td><div class="text-right">Previously Uploaded Big Image</div></td>
                                    <td>
                                        <div class="col-md-9">
                                            <img src="../uploads/presses/<?php echo $big_image; ?>" style="max-width:600px;" />
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="col-md-9">
                                            <a href="<?php echo "../uploads/presses/".$big_image; ?>" download="<?php echo $big_image; ?>" class="btn btn-sm btn-info">Download File</a>
                                            &nbsp; <a href="deletepressfile.php?id=<?php echo $id;?>&filename=<?php echo $big_image; ?>&filetype=big_image&delete=1" class="btn btn-sm btn-danger adelete">Delete File</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php } ?>
                            <tr>
                                ...
                                <td>
                                    <div class="col-md-9">
                                        <input type="file" name="big_image" id="big_image" class="control" /><span class="text-danger">Best Size (width x height): 2000 X 2000 | Best file format JPG</span>
                                    </div>
                                </td>
                            </tr>
                            <tr>
