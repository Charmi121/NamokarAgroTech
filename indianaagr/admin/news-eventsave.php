<?php
    ...
    if (isset($edit)){
        if ((int)$edit == 0){
            $sqlquery = $fpdo->insertInto('tblnews_event', array(
                                            'title' => $photo_title,
											'seo_url' => $seo_url,
                                            'description' => $desc,
											'location' => $location,
            ...
            $sqlquery = $fpdo->update('tblnews_event')->set(array(
                                        'title' => $photo_title,
										'seo_url' => $seo_url,
                                        'description' => $desc,
										'location' => $location,
            ...
            $sqlquery->where('tblnews_event.id = ?', $id);
            $sqlquery->execute();
    ...
    if ((int)$action == 1) {
        ...
        header("Location: displaynews_event.php");
        exit;
    } elseif ((int)$action == 2) {
