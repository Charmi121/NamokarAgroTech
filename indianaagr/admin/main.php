    ...
    final class DBConfig {
        ...
        public function showImagePath($folder_name, $file_name){
            if(!empty($file_name) && (int)$this->checkFileExists(UPLOAD_PATH.DS.$folder_name.DS.$file_name) == 1 ){
                 ...
                 $image_path = "images".DS."no-image.png"; 
            } 
            return $image_path;
        }        
        ...
        public function checkValidDate($datevalue)
        {
        ...
        public function SEOURL($string)
        {
            ...
            $string = preg_replace("`\[.*\]`U","",$string);
            // $string = preg_replace('/(\'|&#0*39;)/', "", $string);  //New
            $string = preg_replace('`&(amp;)?#?[a-z0-9]+;`i','-',$string);
            $string = htmlentities($string, ENT_COMPAT, 'utf-8');
            $string = preg_replace( "`&([a-z])(acute|uml|circ|grave|ring|cedil|slash|tilde|caron|lig|quot|rsquo);`i","\\1", $string );
            $string = preg_replace( array("`[^a-z0-9]`i","`[-]+`") , "-", $string);
        ...
        public function seoURLExist($table_name, $seo_url, $id)
        {
            global $fpdo;
            $result =  array();
            $param = array();
            $slug   =  $this->SEOURL($seo_url);
            // $slug = $seo_url;
            $i = 0;
            ...
            if(!empty($id)) {
                ...
                $sqlquery   =   "SELECT seo_url FROM ".$table_name."";
                $sqlquery   =   $sqlquery . " WHERE seo_url = '".$check_seo_url."'";
                $sqlquery   =   $sqlquery . " AND id != ".$id."";
                $sqlquery   =   $sqlquery . " ORDER BY LENGTH(seo_url) DESC, seo_url DESC LIMIT 1 ";
            } else {
                $check_seo_url  = '^'.$slug.'(-[0-9]+)?$';

                $sqlquery   =   "SELECT seo_url FROM ".$table_name."";
                $sqlquery   =   $sqlquery . " WHERE seo_url REGEXP '".$check_seo_url."'";
                $sqlquery   =   $sqlquery . " ORDER BY LENGTH(seo_url) DESC, seo_url DESC LIMIT 1 ";
            }


            $rsseo      =   $fpdo->customResult($sqlquery)->fetchAll();
            if (count($rsseo)>0)
            {
                foreach($rsseo as $rowseo ) {
                    $slug = $rowseo['seo_url'];
                }
                ...
                return $this->seoURLExist($table_name, $slug, $id);
            } else {
                $result['seo_url']  = $slug;
                $result['status']  = "success";
                return $result;
