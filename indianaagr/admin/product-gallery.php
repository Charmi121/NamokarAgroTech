    ...
    <body>
        ...
        <div class="container">
            ...
            <?php if (!empty($_SESSION['technicalstatus']) && $_SESSION['technicalstatus'] == "invalid") { ?>
                ...
                <?php
                unset($_SESSION['technicalstatus']);
            ...
            <div class="middlesection margin-top20">
                ...
                if (!empty($_GET['product_id'])) {
                    ...
                    if (!empty($id) && !empty($edit)) {
                        $sqlquery = $fpdo->from("tblproduct_gallery_images")
                                ->where("tblproduct_gallery_images.id = :id", array(":id" => $id));
                        $rstechspec = $sqlquery->fetchAll();
                        if (count($rstechspec) > 0) {
                            foreach ($rstechspec as $rowtechspec) {
                                $alt_text = trim($rowtechspec['alt_text']);
                ...
                <table class="table table-condensed">
                    <tbody>
                        <tr>
                            <td>
                                <form name="addform" id="addform" method="post" action="product-gallery-save.php"  enctype="multipart/form-data" target="_self" autocomplete="off">
                                    ...
                                    <table class="table table-bordered table-striped table-condensed">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><div class="text-right">Alt Title</div></td>
                                                <td>
                                                    <div class="col-md-12">
                                                        <input type="text" name="alt_text" id="title" value="<?php echo $alt_text; ?>"  class="form-control" title="Please enter tech title.">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                    ...
                                                    <td>
                                                            <div class="col-md-9">
                                                                    <input type="file" name="big_image_1" id="big_image_1" class="control required" /><span class="text-danger">Best Size (width x height): 900px X 700px</span>
                                                            </div>
                                                    </td>
                                            </tr>
                                            <tr>
                            ...
                            <td colspan="10">
                                <table class="table table-bordered table-striped table-condensed">
                                    ...
                                    <tbody>
                                        ...
                                        $sqlquery = $fpdo->from("tblproduct_gallery_images")
                                                ->where("tblproduct_gallery_images.product_id = :product_id AND status != :status", array(":product_id" => $product_id, ":status" => 909))
                                        ...
                                        if (count($rsprojectkeytech_descriptions) > 0) {
                                            foreach ($rsprojectkeytech_descriptions as $rowprojectkeytech_description) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo $i; ?></td>
                                                    <td><img style="max-width:120px;" src="<?php echo DISPLAY_PATH."/products/".trim($rowprojectkeytech_description['thumb_image']); ?>" /></td>									
                                                    <td><span class="label <?php if ((int) $rowprojectkeytech_description['status'] == 707) {
                                                            echo "label-success";
                                                        } else {
                                                            echo "label-danger";
                                                        } ?>"><?php if ((int) $rowprojectkeytech_description['status'] == 707) {
                                                                echo "Enable";
                                                            } else {
                                                                echo "Disable";
                                                            } ?></span>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myDeleteLabel">Delete Gallery Image ?</h4>
                    </div>
                    <div class="modal-body">
                        <p>You have selected to delete this gallery image.</p>
                        <p>
                    ...
                    <div class="modal-footer">
                        <form name="frmdelete" id="frmdelete" method="post" action="product-gallery-save.php">
                            <input type="hidden" name="delete" id="delete" value="1" />
                            <input type="hidden" name="id" id="id" value="" />
                            <input type="hidden" name="product_id" id="product_id" value="<?php echo $product_id; ?>" />
                            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                            <button type="submit" id="btndelete222" class="btn btn-danger">Delete</button>
                        </form>
                    </div>
                </div>
            </div>
...
<?php require_once("footer.php") ?>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                Admin.Utils.ProductGallery.initProductImage();
                Admin.Utils.ProductGallery.deleteProductImage();
            });
        </script>
    </body>
</html>